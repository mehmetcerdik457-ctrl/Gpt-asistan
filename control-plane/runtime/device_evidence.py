...
def make_template(path):
    ...
    data={'version':1,'status':SCHEMA['status_without_device'],'evidence':evidence}
...
def validate(path):
    ...
    if not all(evidence[x]=='PASS' for x in ('install_result','launch_result','postcondition_result')):
        ...
        raise SystemExit(2)
    print(json.dumps({'status':'REAL_DEVICE_EVIDENCE_VALID','package_name':evidence['package_name']},sort_keys=True))

def main():
    p=argparse.ArgumentParser(); sub=p.add_subparsers(dest='cmd',required=True)
    a=sub.add_parser('template'); a.add_argument('--output',required=True)
    b=sub.add_parser('validate'); b.add_argument('--input',required=True)
    args=p.parse_args()
    make_template(args.output) if args.cmd=='template' else validate(args.input)
